'''
This script assumes you have some JSON file that contains a list of lat longs.

Once the lat longs are read in some QAQC is performed. Primarily, 'neighbors'
are defined so for one location only boulders that have neighbors are 
returned. 
...
from non_dem.trainingDataCreation import modifyImageName, parallelProcessAllPixels,\
     parallelizeMapCreation, singleCreateMap

from non_dem.generateBoundingBox import grabBaseXML, grabImageLabel, \
    generateAllXML, mergePixelsToImages

from non_dem.postprocess import find_empty_images, remove_file
...
def setup(numNeighbors = 30):
    ...
    '''
    Parse lat long -> QAQC -> find neighbor boulders
    '''
